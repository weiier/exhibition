...
function bindEvent() {
    ...
    $("#downloadExc").click(function(){   
        var url = "http://"+publicElement.ip+":"+publicElement.port+"/vmap/download!getfile?jsonstr="
        var data = {
        ...
        $.getJSON(
            url,function(data){
                ...
                location.href = "http://"+publicElement.ip+":"+publicElement.port+"/vmap/total.xls";
            }
        );
    });
...
function mapInit() {
    ...
    map.showBooth = function (obj) {
        ...
        if ( publicElement.login_id == "admin") {
...
function showDetail(uId) {
    $('#myModal').modal("show");
    var url = "http://"+publicElement.ip+":"+publicElement.port+"/vmap/change!unit_proxy?client=824&vkey=FFE58998-B203-B44E-A95B-8CA2D6CBCD65&unit="+uId+"&version="+publicElement.version+"&jsoncallback=?";
    $.getJSON(url,
              function( data ) {
                  if ( !!!data.success ) {
                      return;
